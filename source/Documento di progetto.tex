...
\begin{enumerate}[label=\textbf{RF\arabic*}\;, ref=\textbf{RF\arabic*}]
    ...
    \begin{enumerate}[label=\textbf{\alph*}, ref=\textbf{RF13\alph*}, itemsep=0.5em]
        \item \label{logout} effettuare il logout;
        \item \label{disdire l'abbonamento} disdire l’abbonamento; viene chiesto di confermare la scelta. Se l’utente conferma, il servizio sarà disponibile fino al termine del mese per il quale ha pagato;
        \item \label{cambiare password} cambiare la password; 
        \item \label{lingue} scegliere tra le lingue disponibili.   
    \end{enumerate}
...
\begin{enumerate}[label=\textbf{FE\arabic*}\;, ref=\textbf{FE\arabic*}]
    ...
    Vedi \ref{recupero password}. La prima schermata presenta una componente testuale per inserire la mail alla quale inviare il link. La seconda schermata presenta due aree testuali per inserire e confermare la nuova password.
    ...
    La cronologia di ascolto. Vedi \ref{brani}.
